...
# Format date
ardlf$date <- mdy(format(ardlf$date, format = "%m-%d-%Y"))

# Descriptive statistics ----
ardlf |>  
  summarise(
    across(
      .cols = c(tri, annualised_volatility_open, beta_monthly_leveraged, 
  ...
  pivot_wider(names_from = stat, values_from = value) -> wide_data
...
# Correlation matrix ----
correlation_matrix <- ardlf |> 
  select(tri, annualised_volatility_open, beta_monthly_leveraged, 
         dy, market_cap, pe, liqudity) |> 
...
ardlf <- ardlf |> 
  mutate(
    ln_annualised_vol_open = log(annualised_volatility_open),
    ln_mkt_cap = log(market_cap),
    lntri = log(tri)
  )
...
# ADF test
adf_test <- ardlf |>
  select(ln_annualised_vol_open, lntri, ln_mkt_cap, pe, dy, 
         beta_monthly_leveraged, liqudity) |>
  map(~adf.test(.)) |> 
  map_df(broom::tidy, .id = "Variable")
...
# Combine tests
adf_pp <- bind_cols(adf_test, pp_test) |> 
  select(1:3, 8, 9)

colnames(adf_pp) <- c("Variable", "adf_stat", "adf_pval", "pp_stat", "pp_pval")

# ARDL modeling ----
# Auto ARDL
ardl_mod <- auto_ardl(
  lntri ~ ln_annualised_vol_open + ln_mkt_cap + pe + dy + 
    beta_monthly_leveraged + liqudity, 
  data = ardlf, 
  max_order = 4
)
...
# Bounds t-test
bounds_t_test(uecm_22212000, case = 3, alpha = 0.01)
...
# Time series declaration and plots ----
# Create time series objects
tri <- ts(ardlf$tri, start = c(2013, 1), frequency = 12)
amihud_illiqudity <- ts(ardlf$amihud_illiqudity, start = c(2013, 1), frequency = 12)
turn_over_ratio <- ts(ardlf$turn_over_ratio, start = c(2013, 1), frequency = 12)
bid_ask_spread <- ts(ardlf$bid_ask_spread, start = c(2013, 1), frequency = 12)
beta_monthly_leveraged <- ts(ardlf$beta_monthly_leveraged, start = c(2013, 1), frequency = 12)
dy <- ts(ardlf$dy, start = c(2013, 1), frequency = 12)
market_cap <- ts(ardlf$market_cap, start = c(2013, 1), frequency = 12)
annualised_volatility_open <- ts(ardlf$annualised_volatility_open, start = c(2013, 1), frequency = 12)

# Time series plots
autoplot(tri) + labs(title = "TRI")
autoplot(amihud_illiqudity) + labs(title = "Amihud Illiquidity")
autoplot(turn_over_ratio) + labs(title = "Turnover Ratio")
autoplot(bid_ask_spread) + labs(title = "Bid-Ask Spread")
autoplot(beta_monthly_leveraged) + labs(title = "Beta Monthly Leveraged")
autoplot(dy) + labs(title = "Dividend Yield")
autoplot(market_cap) + labs(title = "Market Cap")
autoplot(annualised_volatility_open) + labs(title = "Annualized Volatility")
