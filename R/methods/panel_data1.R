...
# Compare models using residual sum of squares and adjusted R-squared
# These are already shown in summary(), but you can extract them:

# Get model statistics
data.frame(
  Model = c("Pooled", "Fixed", "Random"),
  RSS = c(
    sum(residuals(pooled_model)^2),
    sum(residuals(fixed_model)^2),
    sum(residuals(random_model)^2)
  ),
  R_squared = c(
    summary(pooled_model)$r.squared[1],
    summary(fixed_model)$r.squared[1],
    summary(random_model)$r.squared[1]
  ),
  Adj_R_squared = c(
    summary(pooled_model)$r.squared[2],
    summary(fixed_model)$r.squared[2],
    summary(random_model)$r.squared[2]
  )
)
