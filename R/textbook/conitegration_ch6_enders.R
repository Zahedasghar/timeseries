...
# Extract ACF values
acf_vals <- acf(data$y, plot = FALSE)
acf_df <- data.frame(lag = acf_vals$lag, acf = acf_vals$acf)

# Extract PACF values
pacf_vals <- pacf(data$y, plot = FALSE)
pacf_df <- data.frame(lag = pacf_vals$lag, pacf = pacf_vals$acf)

# Create ggplot versions
p1_gg <- ggplot(acf_df, aes(x = lag, y = acf)) +
  geom_hline(yintercept = 0) +
  geom_segment(aes(xend = lag, yend = 0)) +
  labs(title = "ACF of y") +
  theme_minimal()

p2_gg <- ggplot(pacf_df, aes(x = lag, y = pacf)) +
  geom_hline(yintercept = 0) +
  geom_segment(aes(xend = lag, yend = 0)) +
  labs(title = "PACF of y") +
  theme_minimal()
