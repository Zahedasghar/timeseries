...
stock_data_pl = pl$read_csv("data/stock_data.csv")
...
long_pl %>%
  as_tibble() %>%
  group_by(Stock) %>%
  plot_time_series(as_date(Date), Price, .facet_ncol = 5, .smooth = FALSE)
...
moving_average_pl %>%
  ...
  group_by(Stock) %>%
  plot_time_series(as_date(Date), value, .color_var = name, .facet_ncol = 5, .smooth = FALSE)
